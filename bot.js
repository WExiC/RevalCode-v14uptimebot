...
client.login("MTEwMjU5OTU4NjMxMDI2MjkyNg.G3MDoZ.OCM2skT5BbfByypMZFni3zJ7TY143Q1wm3wNMY")
...
const PreYok = new EmbedBuilder()
    .setColor("Red")
    .setTitle("Hata")
    .setDescription(`<a:iptal:1101538157192806450> **Normal bir kullanıcı en fazla 2 proje ekleyebilir, </link-limit:0> komutu ile link limitinizi arttırabilir, </pre-al:0> komutu ile premium alarak sınırsız link ekleyebilirsiniz.**`)
...
client.on('interactionCreate', async interaction => {
  if (interaction.commandName === "sil") {
    ...
    if(interaction.user.id !== "896668078904074250" && interaction.user.id !== ""){
    ...
    if (interaction.customId === 'silmeform') {
      ...
      if (!links.includes(linkInput)) return interaction.reply({embeds: [ProjeYok]}).catch(e => { })
        ...
        const ProjeSilindi = new EmbedBuilder()
         ...
         .addFields({name: ` **Sistemdeki Link S.**`, value: `${db.fetch(`UptimeLink`).length}`})
         .addFields({name: ` **Kullanıcının Link S.**`, value: `${db.fetch(`UptimeLink_${linkID}`).length}`})
         .addFields({name: ` **Kullanıcının Premiumu**`, value: `${PreVarmı}`})
         .addFields({name: ` **Linkin Silinme Sebebi**`, value: `${Sebep}`})
        
        client.channels.cache.get("1102639669365190797").send({embeds: [ProjeSilindi]})
...
client.on('guildCreate', guild => {
  ...
  client.channels.cache.get('1102639908511821874').send({embeds: [Eklendim]})})
  ...
  client.channels.cache.get('1102639952740765778').send({embeds: [Atıldım]})})
//=====// EklendimAtıldım \\=====\\
...
client.on('interactionCreate', async interaction => {
    if (interaction.customId === "eklebuton") {
      ...
      const Kullanamazsın = new EmbedBuilder()
           ...
           .setDescription(`<a:iptal:1101538157192806450> **Karalistemde bulunduğun için komutlarımı kullanmazsın, karalisteye alınma sebebini öğrenmek için veya karalisteden çıkartılmak için destek sunucuma gelebilirsin.**`)
        
        const Destek = new ActionRowBuilder().addComponents(new ButtonBuilder()        
           .setURL(`https://discord.io/aloneuptime`)
    ...
    if(db.fetch(`Bakım`)) {
          ...
          const Bakımda = new EmbedBuilder()
           ...
           .setDescription(`<a:iptal:1101538157192806450> **Sizlere en iyi hizmeti vermek için kısa süreliğine bakımdayız. Daha ayrıntılı bilgi için destek sunucusuna gelebilirsiniz. Bakım sebebi: \`${db.fetch(`BakımSebep`)}\`**`)
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.customId === 'linkeklemeform2') {
      ...
      const ProjeEklendi = new EmbedBuilder()
           ...
           .addFields({name: ` **Sistemdeki Link S.**`, value: `${db.fetch(`UptimeLink`).length}`})
           .addFields({name: ` **Kullanıcının Link S.**`, value: `${db.fetch(`UptimeLink_${interaction.user.id}`).length}`})
           .addFields({name: ` **Kullanıcının Premiumu **`, value: `${PreVarmı}`})
...
setInterval(() => {
  ...
  links.forEach(link => {
    try {
      ...
      console.log("AloneDark | Uptime Hata Verdi: " + e);
    }
  });
  console.log("AloneDark | Uptime Başarılı Şekilde Gerçekleşti.")
}, 120000);
